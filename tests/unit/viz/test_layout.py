...
from cartoframes.viz.sources import GeoDataFrameSource
...
class TestLayoutInitialization(object):
    ...
    def test__init_maps(self):
        """Layout should init properly"""
        layout = Layout([Map(Layer(GeoDataFrameSource(SOURCE))),
                         Map(Layer(GeoDataFrameSource(SOURCE)))])
    ...
    def test__init_maps_valid(self):
        ...
        with pytest.raises(Exception) as e:
            Layout([Layer(GeoDataFrameSource(SOURCE))])
        assert str(e.value) == msg

    def test__init_maps_size(self):
        """Layout should init properly"""
        layout = Layout([Map(Layer(GeoDataFrameSource(SOURCE))),
                         Map(Layer(GeoDataFrameSource(SOURCE)))], 1, 2)
...
class TestLayoutSettings(object):
    ...
    def test_custom_viewport(self):
        ...
        layout = Layout([
            Map(Layer(GeoDataFrameSource(SOURCE)), viewport={'zoom': 2}),
            Map(Layer(GeoDataFrameSource(SOURCE)))
        ], viewport={'zoom': 5})
