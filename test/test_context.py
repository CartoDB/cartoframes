...
class TestCartoContext(unittest.TestCase, _UserUrlLoader):
    ...
    def test_cartocontext_write_points_dataset(self):
        ...
        self.test_write_table = cc.write(df, self.test_write_table)
    ...
    def test_cartocontext_write_lnglat_dataset(self):
        ...
        self.test_write_table = cc.write(df, self.test_write_table, lnglat=('dropoff_longitude', 'dropoff_latitude'))
    ...
    def test_cartocontext_write_null_geometry_column(self):
        ...
        result = self.sql_client.send('SELECT * FROM {} WHERE the_geom IS NULL'.format(self.test_write_table))
        self.assertEqual(result['total_rows'], 100)
    ...
    def test_cartocontext_write_geopandas(self):
        ...
        self.test_write_table = cc.write(gdf, self.test_write_table)
    ...
    #     from cartoframes.examples import read_taxi
    #     df = read_taxi(limit=100)
    #     self.test_write_table = cc.write(df, self.test_write_table, privacy='link')

    #     self.assertExistsTable(self.test_write_table)

    #     dataset = DatasetManager(self.auth_client).get(self.test_write_table)
