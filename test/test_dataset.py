...
class TestDataset(unittest.TestCase, _UserUrlLoader):
    ...
    def setUp(self):
        ...
        self.c = Context(base_url=self.baseurl, api_key=self.apikey)
    ...
    def tearDown(self):
        ...
        if self.apikey and self.baseurl:
            for table in tables:
                try:
                    self.c.delete(table)
                    self.c.sql_client.send(sql_drop.format(table))
    ...
    def test_dataset_from_table(self):
        table_name = 'fake_table'
        dataset = Dataset.from_table(table_name=table_name, context=self.c)
    ...
    def test_dataset_from_query(self):
        query = 'SELECT * FROM fake_table'
        dataset = Dataset.from_query(query=query, context=self.c)
    ...
    def test_dataset_download_validations(self):
        ...
        dataset = Dataset.from_table(table_name=self.test_write_table, context=self.c)
        df = dataset.download()
        self.assertEqual('fakec' in df.columns, True)
    ...
    def test_dataset_write_points_dataset(self):
        ...
        dataset = Dataset.from_dataframe(df).upload(table_name=self.test_write_table, context=self.c)
        self.test_write_table = dataset.table_name

        result = self.c.sql_client.send('SELECT * FROM {} WHERE the_geom IS NOT NULL'.format(self.test_write_table))
    ...
    def test_dataset_write_lnglat_dataset(self):
        ...
        dataset = Dataset.from_dataframe(df).upload(
            with_lnglat=lnglat, table_name=self.test_write_table, context=self.c)
        self.test_write_table = dataset.table_name
    ...
    def test_dataset_write_null_geometry_column(self):
        ...
        result = self.c.sql_client.send('SELECT * FROM {} WHERE the_geom IS NULL'.format(self.test_write_table))
        self.assertEqual(result['total_rows'], 100)
    ...
    def test_dataset_write_geopandas(self):
        ...
        dataset = Dataset.from_dataframe(gdf).upload(table_name=self.test_write_table, context=self.c)
        self.test_write_table = dataset.table_name
    ...
    def test_dataset_write_if_exists_append(self):
        ...
        dataset = Dataset.from_dataframe(df).upload(
            if_exists=Dataset.APPEND, table_name=self.test_write_table, context=self.c)
    ...
    def test_dataset_write_if_exists_replace(self):
        ...
        dataset = Dataset.from_dataframe(df).upload(
            if_exists=Dataset.REPLACE, table_name=self.test_write_table, context=self.c)
    ...
    def test_dataset_schema_from_parameter(self):
        schema = 'fake_schema'
        dataset = Dataset.from_table(table_name='fake_table', schema=schema, context=self.c)
        self.assertEqual(dataset._schema, schema)

    def test_dataset_schema_from_non_org_context(self):
        dataset = Dataset.from_table(table_name='fake_table', context=self.c)
        self.assertEqual(dataset._schema, 'public')
    ...
    #     dataset = self.c.write(df, self.test_write_table)
    #     self.test_write_table = dataset.table_name

    #     self.assertExistsTable(self.test_write_table)

    def assertExistsTable(self, table_name):
        resp = self.c.sql_client.send('''
            SELECT *
            FROM {table}
            LIMIT 0
