...
def enrich_polygons(data, variables, agg_operators, data_geom_column='geometry', filters=dict(), credentials=None):
    ...
    sql = __prepare_sql(_ENRICHMENT_ID, variables_list, agg_operators, table_data_enrichment,
                        table_geo_enrichment, credentials.username.replace('-', '_'), _WORKING_PROJECT,
                        user_workspace, data_tablename, data_geom_column, filters_str)
...
def __prepare_sql(enrichment_id, variables, agg_operators, enrichment_table, enrichment_geo_table, user_workspace,
    ...
    '''.format(enrichment_id=enrichment_id, variables=', '.join(variables_sql),
               ...
               user_workspace=user_workspace, working_project=working_project,
