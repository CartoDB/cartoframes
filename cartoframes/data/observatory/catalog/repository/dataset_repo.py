from .constants import CATEGORY_FILTER, COUNTRY_FILTER, GEOGRAPHY_FILTER, PROVIDER_FILTER, PUBLIC_FILTER
...
_ALLOWED_FILTERS = [CATEGORY_FILTER, COUNTRY_FILTER, GEOGRAPHY_FILTER, PROVIDER_FILTER, PUBLIC_FILTER]
...
class DatasetRepository(EntityRepository):
    ...
    def get_all(self, filters=None, credentials=None, default_credentials=None):
        if credentials is not None:
            filters = self._add_subscription_ids(filters, credentials, DATASET_TYPE)
            if filters is None:
                return []

        elif credentials is None and default_credentials is not None:
