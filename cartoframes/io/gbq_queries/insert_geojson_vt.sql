INSERT INTO `{output_table}` (z, x, y, mvt, quadkey)
    WITH tile_bbox AS (
        SELECT carto_tiler.ST_TileEnvelope_bbox({xmin}, {ymin}, {xmax}, {ymax}, z) AS tiles
          FROM UNNEST(GENERATE_ARRAY({geojson_vt_base_zoom}, {geojson_vt_base_zoom}, 1)) z
    ),
    tile_list AS (
        SELECT tiles.z AS z, x, y
            FROM tile_bbox,
                UNNEST(GENERATE_ARRAY(tiles.xmin, tiles.xmax, 1)) x,
                UNNEST(GENERATE_ARRAY(tiles.ymin, tiles.ymax, 1)) y
    ),
    tiles AS (
        SELECT t.z AS __z, t.x AS __x, t.y AS __y,
                t.xmin AS __xmin, t.xmax AS __xmax, t.ymin AS __ymin, t.ymax AS __ymax
            FROM (
    ...
    inter AS (
        SELECT b.__z AS z, b.__x AS x, b.__y AS y, a.__geoid AS geoid
            ...
            WHERE NOT ((a.__xmin > b.__xmax) OR (a.__xmax < b.__xmin) OR (a.__ymax < b.__ymin) OR (a.__ymin > b.__ymax))
    ),
    nested_tiles AS (
